...
def evaluate_pristine_oos(
    ...
    baseline_probability = [base_rate] * len(target)

    print()
    print("Pristine OOS evaluation (locked baseline):")
    print("  Training: full development set only")
    print(
        f"  Development period: {development_binary['setup_timestamp'].min()} "
        f"-> {development_binary['setup_timestamp'].max()}"
    )
    print(
        f"  OOS period:         {oos['setup_timestamp'].min()} "
        f"-> {oos['setup_timestamp'].max()}"
    )
    print(f"  Development rows:   {len(development_binary):,}")
    print(f"  OOS rows:            {len(oos):,}")
    print(f"  Model features:      {len(model_columns):,}")
    print(f"  OOS ROC-AUC:         {roc_auc_score(target, probability):.4f}")
    print(f"  OOS log loss:        {log_loss(target, probability):.4f}")
    print(
        f"  Baseline log loss:   "
        f"{log_loss(target, baseline_probability):.4f}"
    )
