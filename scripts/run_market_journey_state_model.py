"""Compact empirical Market Journey state model conditioned on BOS scope.

Descriptive research only. No signal, ranking, score, or winner is produced.
Uses the validated BOS-scope context and H40 journey episodes.
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("data/research/xauusd_m30_market_journey_state_model.csv"),
    )
    ...
    for (period, direction, bos_scope, state), group in episode.groupby(
    ...
    if report.empty:
        ...
        for (period, direction, bos_scope, state), group in report.groupby(
            ["period", "direction", "bos_scope", "state"]
        ):
            print(f"{period} {direction} {bos_scope} {state}")
            print(
                group[
                    [
                        "next_state",
