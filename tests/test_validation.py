...
def test_time_series_split_is_chronological_and_purged() -> None:
    ...
    assert list(range(fold.train_start, fold.train_end)) == list(range(67))
    assert list(range(fold.validation_start, fold.validation_end)) == list(range(77, 97))
    assert list(range(fold.test_start, fold.test_end)) == list(range(97, 100))
